...
function TabIcon({ name, focused, label }) {
  return (
    <View className="items-center justify-center pt-1">
      ...
      />
      <Text className={`text-xs mt-0.5 ${focused ? "text-primary font-medium" : "text-outline"}`}>
        {label}
      </Text>
    </View>
  );
...
export default function TabsLayout() {
  ...
  return (
    ...
    >
      ...
      <Tabs.Screen
        name="create"
        options={{ tabBarIcon: ({ focused }) => <TabIcon name="add-circle" focused={focused} label="Create" /> }}
      />
      <Tabs.Screen
        name="profile"
